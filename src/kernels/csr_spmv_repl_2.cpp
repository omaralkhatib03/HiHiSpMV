...
void read_rows(
    ...
    #pragma HLS BIND_STORAGE variable=row_ptr type=ram_2p impl=BRAM
...
#define TILE_BLOCKS 5
    ...
    tiles:
        for (unsigned int tile=0; tile<tiles; tile++) {
            ...
            #pragma HLS BIND_STORAGE variable=vector type=RAM_1WNR impl=URAM
            #pragma HLS ARRAY_RESHAPE variable=vector type=cyclic factor=1
            // #pragma HLS ARRAY_PARTITION variable=vector type=cyclic factor=2
            vec_read: 
...
extern "C" {
    ...
    void csr_spmv_repl_2(
        ...
        const unsigned int str_depth = 4096; //
